...
Coaches previously had to create a weekly recurring schedule one weekday at a time, capped at 16 weeks, with no way to pause a schedule without deleting it. Schedules now continue until explicitly ended. The app keeps a rolling 16-week window of sessions materialized whenever staff load the practice schedule, while the recurring template itself does not expire.

## What changed

- **`practice_series.active` is now toggleable.** A new `Jadwal berulang` page (`/latihan/jadwal`) lists every recurring schedule — active and inactive — grouped by (title, time, location), with one button per weekday that toggles that specific day on/off. Turning a day off stops it from generating new sessions; it does **not** retroactively cancel sessions already scheduled (those still use the existing "Batalkan sesi" action or the date-range skip tool). Turning a day back on immediately re-materializes its rolling window from the later of today or the schedule's original start date.
- **Ended schedules stay historical.** “Batalkan sesi ini dan berikutnya” ends the series and marks it as ended on the management page. It cannot be mistaken for a paused schedule or switched back on; create a new schedule to resume after an intentional end.
- **Multi-day creation.** The "Jadwal berulang setiap minggu" section in the practice editor now has a weekday toggle group instead of a single dropdown. Selecting several days in one submission creates one independent series per day (so each day can later be turned on/off separately) sharing the same title/time/location. The server creates the whole selection in one transaction, so a failure cannot leave only some weekdays saved.
