...
export function installPreviewHostBridge(options: PreviewHostBridgeOptions = {}): () => void {
  ...
  const onMessage = (event: MessageEvent) => {
    ...
    if (type === "hello" && HelloSchema.safeParse(event.data).success) announce();
    if (type === "navigate") {
      const parsed = NavigateSchema.safeParse(event.data);
      if (parsed.success) { navigate(parsed.data.path); queueMicrotask(reportLocation); }
    }
    if (type === "history") {
    ...
    if (type === "connector-token-ready" && ConnectorTokenReadySchema.safeParse(event.data).success) {
