...
export type GateIdentity = { sub: string; email: string | null; name: string | null; teamId: string | null };
...
export function gateKeyResolver(url: string, jwksFetch: JwksFetch = defaultJwksFetch): JWTVerifyGetKey {
...
export type VerifyGateIdentityTokenOptions = { issuer: string; audience: string; getKey: JWTVerifyGetKey };
...
export async function verifyGateIdentityToken(
  ...
  try {
    const { payload } = await jwtVerify(token, options.getKey, {
      algorithms: ["EdDSA"], issuer: options.issuer, audience: options.audience,
      requiredClaims: ["sub", "iat", "exp"], maxTokenAge: "10 minutes",
    });
...
export function resolveGateEndpoints(headers: Headers): GateEndpoints | null {
  ...
  if (isWorkspacePreview()) {
    return { issuer: PREVIEW_GATE_ORIGIN, jwksUrl: `${PREVIEW_GATE_ORIGIN}${GATE_JWKS_PATH}` };
  }
...
export async function gateIdentityFromHeaders(headers: Headers, jwksFetch?: JwksFetch): Promise<GateIdentity | null> {
...
export type GateUserInfo = { id: string; email: string; emailVerified: boolean; name: string };
