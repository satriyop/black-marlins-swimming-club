...
export async function deleteResult(actor: Actor, id: number): Promise<{ ok: true }> {
  ...
  const rows = await actor.sql<{ swimmer_id: number }>`
    select swimmer_id from results where id = ${id} and club_id = ${clubId} limit 1
  `;
...
export async function saveMeetRecord(
  ...
  const rows = await actor.sql<{ id: number }>`insert into meets (club_id, name, level, course, venue, city, start_date, end_date, organizer, status, notes) values (${clubId}, ${data.name.trim()}, ${data.level}, ${data.course}, ${data.venue?.trim() || null}, ${data.city?.trim() || null}, ${data.startDate}, ${data.endDate || null}, ${data.organizer?.trim() || null}, ${data.status}, ${data.notes?.trim() || null}) returning id`;
  return { id: rows[0]!.id };
}

export async function saveMeetEntry(
  actor: Actor,
  data: { meetId: number; swimmerId: number; stroke: string; distanceM: number; seedTimeMs?: number | null; ageGroup: string },
): Promise<{ id: number }> {
  const hats = await hatsFor(actor);
  if (!canWriteMeetEntry(hats, data.swimmerId)) throw new Error("Tidak diizinkan");
  const clubId = await requireClubId(actor);
  const rows = await actor.sql<{ id: number }>`insert into meet_entries (club_id, meet_id, swimmer_id, stroke, distance_m, age_group, seed_time_ms, status) values (${clubId}, ${data.meetId}, ${data.swimmerId}, ${data.stroke}, ${data.distanceM}, ${data.ageGroup}, ${data.seedTimeMs ?? null}, 'terdaftar') returning id`;
  return { id: rows[0]!.id };
}
