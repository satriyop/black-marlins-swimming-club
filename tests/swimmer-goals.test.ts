...
test("a goal only advances from completed times with the same stroke, distance and pool length", async () => {
  ...
  const addResult = async (course: "25" | "50", timeMs: number, status = "selesai", stroke = "bebas") => {
    await f.h.sql`
  ...
  [goal] = await listSwimmerGoals(f.guardian, f.own);
  expect(goal).toMatchObject({ status: "hit", bestTimeMs: 34_500, deltaMs: -500, hitOn: todayIso() });
...
test("guardians can read only linked children's goals and cannot set or edit them", async () => {
  ...
  expect((await listSwimmerGoals(f.guardian, f.own)).map((goal) => goal.notes))
    .toEqual(["Fokus start dan putaran."]);
  ...
  await expect(saveSwimmerGoal(f.otherGuardian, { ...goalInput(f.other), id: own.id }))
    .rejects.toThrow("Tidak diizinkan");
  await expect(saveSwimmerGoal(f.coach, { ...goalInput(f.other), id: own.id }))
...
test("a missed deadline stays missed when a faster result is recorded afterwards", async () => {
  ...
  expect(goal).toMatchObject({ id: rows[0]!.id, status: "missed", bestTimeMs: null, hitOn: null });
  await f.h.sql`
    insert into results (club_id,swimmer_id,result_date,stroke,distance_m,course,time_ms,status,kind)
    values (${f.clubId},${f.own},${dayOffset(-2)},'bebas',50,'50',34900,'selesai','official')
  `;
  [goal] = await listSwimmerGoals(f.coach, f.own);
  expect(goal).toMatchObject({ status: "hit", bestTimeMs: 34_900, hitOn: dayOffset(-2) });
});

test("coach can correct a goal, but invalid or foreign-club inputs cannot write it", async () => {
  const f = await fixture();
  const { id } = await saveSwimmerGoal(f.coach, goalInput(f.own));
  await saveSwimmerGoal(f.coach, { ...goalInput(f.own), id, targetTimeMs: 34_000, notes: "  Lebih cepat  " });
  expect((await listSwimmerGoals(f.coach, f.own))[0])
    .toMatchObject({ targetTimeMs: 34_000, notes: "Lebih cepat" });
  await expect(saveSwimmerGoal(f.coach, { ...goalInput(f.own), course: "25", deadline: dayOffset(-1) }))
    .rejects.toThrow("Batas waktu");
  await expect(saveSwimmerGoal(f.coach, { ...goalInput(f.own), targetTimeMs: -1 }))
    .rejects.toThrow("Waktu target");
  await expect(saveSwimmerGoal(f.coach, { ...goalInput(f.own), notes: "x".repeat(501) }))
  ...
  `;
  await expect(saveSwimmerGoal(f.coach, goalInput(child[0]!.id)))
